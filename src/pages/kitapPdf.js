...
export async function testPdfOlustur(sorular, ilerlemeCallback, testBasligi, ustKirpmaOrani = 0) {
  ...
  for (let i = 0; i < sorular.length; i++) {
    ...
    if (etiketVar) {
      // Kullanıcı isteğiyle (2. tur — "daha koyu ve kalın, belirgin olsun"):
      // ÖNCEDEN ince/gri (107,114,128) idi; sonra siyah+kalın 11pt yapıldı,
      // kullanıcı hâlâ daha belirgin istediği için punto da büyütüldü (13pt).
      const etiket = `${siraNo}.`
      doc.setFontSize(13)
      doc.setFont(undefined, 'bold')
      doc.setTextColor(0, 0, 0)
