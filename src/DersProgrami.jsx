...
export default function DersProgrami() {
  ...
  function veriyiYenile() {
    ...
    Promise.all([
      ...
      // bire bir atamalarını bu öğrenci id'si üzerinden çekeceğiz.
      isVeliYaDaOgrenci
        ? supabase.from('ogrenciler').select('id, ad_soyad').or(`veli_profile_id.eq.${profile.id},ogrenci_profile_id.eq.${profile.id}`)
        : Promise.resolve({ data: [] }),
    ]).then(([p, s, og, ba, by, o, kendiCocuklar]) => {
      setProgram(
        (p.data || []).map((d) => ({
          ...d,
      ...
      const cocukListesi = kendiCocuklar.data || []
